...
let UserService = class UserService {
    usersRepository;
    uploadService;
    constructor(usersRepository, uploadService) {
    ...
    async getOneUser(id) {
        ...
        if (!user)
            throw new common_1.NotFoundException('no user found for this id');
        return user;
    }
    ...
    async updateMe(userId, body) {
        const user = await this.usersRepository.findOneBy({ id: userId });
        if (!user)
            throw new common_1.NotFoundException("No user foun for this id");
